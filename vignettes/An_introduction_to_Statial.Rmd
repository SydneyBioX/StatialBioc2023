...
<img src="images/KontextSpace.png" alt="Kontext" style="width 3in; border: 0px"/>
...
First, let's examine the same effect observed earlier with Kontextual. To avoid redifing cell types we'll examine the distance between p53-positive tumour cells and macrophages in the context of total keratin/tumour cells for image 6.
...
Statial provides a convenient function for observing this interaction - `plotStateChanges`. Here, we can specify the `imageID` and our main cell of interest, `primaryCellType` and our interacting cell type `otherCellType` and our marker of interest `marker`, in this case p53.
...
Beyond looking at single cell-to-cell interactions for a single image, we can also look at all interactions across all images. Statial provides the `getStateChanges` function for this exact purpose, allowing us to examine the most significant correlations between distance and marker expression. Here, we will focus on the most significant interactions in image 6 of the Keren et al. breast cancer dataset.
...
Could this potentially be an artefact of `getStateChanges`? We can examine the image through the `plotStateChanges` function, where we indeed observe an apparent localisation between B cells and T cells.
...
The next steps will involve comparing survival time to see if these distance-marker interactions are practically useful.
