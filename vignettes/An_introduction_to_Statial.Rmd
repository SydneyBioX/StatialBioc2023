...
3.  Why are the majority of most significant interactions occurring in image 35? Configure the parameters of `plotStateChanges` to examine some these most significant interactions.
:::
...
Similiar to `Kontextual`, we can run a similar survival analysis using our state changes results. Here, `prepMatrix` extracts the coefficients, or `coef` column of stateChanges by default. To use the t values instead, specify `column = "tval"` in the `prepMatrix` function.
